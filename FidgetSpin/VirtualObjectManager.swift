...
class VirtualObjectManager {
    ...
    func loadVirtualObject(_ object: VirtualObject, to position: SCNVector3, cameraTransform: matrix_float4x4) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            object.loadModel(from: "art.scnassets/fidgetSpinner.scn")
    ...
    func translate(_ object: VirtualObject, in sceneView: ARSCNView, basedOn screenPos: CGPoint, instantly: Bool, infinitePlane: Bool) {
        DispatchQueue.main.async {
            let result = self.worldPositionFromScreenPosition(screenPos, in: sceneView, objectPos: object.worldPosition, infinitePlane: infinitePlane)
    ...
    func setPosition(for object: VirtualObject, position: SCNVector3, instantly: Bool, filterPosition: Bool, cameraTransform: matrix_float4x4) {
        if instantly {
    ...
    private func setNewVirtualObjectPosition(_ object: VirtualObject, to pos: SCNVector3, cameraTransform: matrix_float4x4) {
        let cameraWorldPos = cameraTransform.translation
        var cameraToPosition = pos - cameraWorldPos
        
        // Limit the distance of the object from the camera to a maximum of 10 meters.
        
        if cameraToPosition.length() > Float(10.0) {
            cameraToPosition = cameraToPosition.normalized()
            cameraToPosition *= 10
        }
        
        object.worldPosition = cameraWorldPos + cameraToPosition
        object.recentVirtualObjectDistances.removeAll()
    }
    
    private func updateVirtualObjectPosition(_ object: VirtualObject, to pos: SCNVector3, filterPosition: Bool, cameraTransform: matrix_float4x4) {
        let cameraWorldPos = cameraTransform.translation
        var cameraToPosition = pos - cameraWorldPos
        
        // Limit the distance of the object from the camera to a maximum of 10 meters.
        if cameraToPosition.length() > 10 {
            cameraToPosition = cameraToPosition.normalized()
            cameraToPosition *= 10
        }
        ...
        let hitTestResultDistance = cameraToPosition.length()
        ...
        if filterPosition, let averageDistance = object.recentVirtualObjectDistances.average {
            let averagedDistancePos = cameraWorldPos + cameraToPosition.normalized() * averageDistance
            object.worldPosition = averagedDistancePos
        } else {
            object.worldPosition = cameraWorldPos + cameraToPosition
        }
    }
    ...
    func transform(for object: VirtualObject, cameraTransform: matrix_float4x4) -> (distance: Float, rotation: Int, scale: Float) {
        ...
        let vectorToCamera = cameraPos - object.worldPosition
    ...
    func worldPositionFromScreenPosition(_ position: CGPoint,
        ...
        let dragOnInfinitePlanesEnabled = false // UserDefaults.standard.bool(forKey: .dragOnInfinitePlanes)
