...
//  WayfairCore
//
//  Created by Johan Ospina on 9/6/17.
//
...
// MARK: - Movement Restriction

enum DirectionRestriction {
    case noRestrictions, nothingHigher, nothingLower, dontChange
}

struct MovementRestriction {
    let xRestriction: DirectionRestriction
    let yRestriction: DirectionRestriction
    let zRestriction: DirectionRestriction
    
    func restrict(newPosition: SCNVector3, withExistingPosition existingPosition: SCNVector3) -> SCNVector3 {
        var restrictedPosition = newPosition
        
        restrictedPosition.x = restrictDimension(newDimension: restrictedPosition.x,
                                                 withRestriction: self.xRestriction,
                                                 toExistingDimension: existingPosition.x)
        
        restrictedPosition.y = restrictDimension(newDimension: restrictedPosition.y,
                                                 withRestriction: self.yRestriction,
                                                 toExistingDimension: existingPosition.y)
        
        restrictedPosition.z = restrictDimension(newDimension: restrictedPosition.z,
                                                 withRestriction: self.zRestriction,
                                                 toExistingDimension: existingPosition.z)
        
        return restrictedPosition
    }
    
    fileprivate func restrictDimension(newDimension: Float, withRestriction restriction: DirectionRestriction, toExistingDimension existingDimension: Float) -> Float {
        guard restriction != .noRestrictions && newDimension != existingDimension else {
            return newDimension
        }
        
        guard restriction != .dontChange else {
            return existingDimension
        }
        
        // If changing to a higher value is restricted, make sure the new dimension is lower
        // If changing to a lower value is restricted, make sure the new dimension is higher
        if restriction == .nothingHigher && existingDimension < newDimension {
            return existingDimension
        } else if restriction == .nothingLower && existingDimension > newDimension {
            return existingDimension
        }
...
extension SCNNode {
    ...
    var eulerAnglesDegrees: SCNVector3 {
        let conversionMultiplier = 180.0 / Float.pi
        return SCNVector3(eulerAngles.x * conversionMultiplier, eulerAngles.y * conversionMultiplier, eulerAngles.z * conversionMultiplier)
    }
}
...
extension SCNVector3 {
    ...
    static func lerp(a: SCNVector3, b: SCNVector3, t: Float) -> SCNVector3 {
        let xLerp = lerpf(a: a.x, b: b.x, t: t)
        let yLerp = lerpf(a: a.y, b: b.y, t: t)
        let zLerp = lerpf(a: a.z, b: b.z, t: t)
        
        return SCNVector3(x: xLerp, y: yLerp, z: zLerp)
    }
